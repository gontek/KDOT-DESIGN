CREATE OR REPLACE TRIGGER pontis.taur_ds_BRIDGE_FACILITY
   after insert or update of FACILITY on pontis.BRIDGE for each row
...
-- Generated 2002-02-26 at 09:26
 ...
 DISABLE WHEN (nvl( new.FACILITY, '<MISSING>' ) <>  nvl( old.FACILITY, '<MISSING>' )) declare
   lb_result boolean;
...
begin
    ...
    lb_result := ksbms_pontis.f_pass_update_trigger_params(
      ...
      'BRIDGE',
      'FACILITY',
      TRIM(UPPER(:old.FACILITY)),
      TRIM(UPPER(:new.FACILITY)),
      1,
