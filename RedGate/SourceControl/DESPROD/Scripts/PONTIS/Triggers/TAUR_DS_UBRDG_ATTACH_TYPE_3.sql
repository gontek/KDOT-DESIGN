CREATE OR REPLACE Trigger pontis.TAUR_DS_UBRDG_ATTACH_TYPE_3
   after insert or update of ATTACH_TYPE_3 on pontis.USERBRDG for each row
...
when (nvl( new.ATTACH_TYPE_3, '<MISSING>' ) <>  nvl( old.ATTACH_TYPE_3, '<MISSING>' )AND
     (NEW.ATTACH_TYPE_3 NOT IN ('0','_') ))
declare
   lb_result boolean;
