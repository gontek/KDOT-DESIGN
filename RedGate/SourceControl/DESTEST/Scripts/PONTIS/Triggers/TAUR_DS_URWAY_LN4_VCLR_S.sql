CREATE OR REPLACE TRIGGER pontis.TAUR_DS_URWAY_LN4_VCLR_S
   after insert or update of LN4_VCLR_S on pontis.USERRWAY for each row

-----------------------------------------------------------------------------------------------------------------------------------------
-- Trigger: taur_ds_URWAY_LN4_VCLR_S
...
-- Generated 2006-07-12
 ...
 WHEN (nvl( new.LN4_VCLR_S, -9 ) <>  nvl( old.LN4_VCLR_S, -9 ) ) declare
  lb_result    boolean;
  ls_bridge_id bridge.bridge_id%type;
begin

  -- new code
  IF Ksbms_Util.f_Numbers_Differ(Nvl(:Old.LN4_VCLR_S, -9),
                                 Nvl(:New.LN4_VCLR_S, -9),
  ...
  lb_result    := ksbms_pontis.f_pass_update_trigger_params(nvl(:new.brkey,
                                                            ...
                                                            'USERRWAY',
                                                            'LN4_VCLR_S',
                                                            TO_CHAR(:old.LN4_VCLR_S),
                                                            TO_CHAR(:new.LN4_VCLR_S),
                                                            6,
                                                            nvl(ls_bridge_id,
                                                                '<MISSING>'),
                                                            'taur_ds_URWAY_LN4_VCLR_S');
  if lb_result then
    dbms_output.put_line('taur_ds_URWAY_LN4_VCLR_S failed');
  end if;
end taur_ds_URWAY_LN4_VCLR_S;
/