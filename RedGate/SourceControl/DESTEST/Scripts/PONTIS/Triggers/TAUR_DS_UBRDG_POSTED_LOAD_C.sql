CREATE OR REPLACE Trigger pontis.TAUR_DS_UBRDG_POSTED_LOAD_C
   after insert or update of POSTED_LOAD_C on pontis.USERBRDG for each row
...
when (nvl( new.POSTED_LOAD_C, '-9' ) <>  nvl( old.POSTED_LOAD_C, '-9' ) )
declare
  lb_result    boolean;
...
begin
  ...
  lb_result    := ksbms_pontis.f_pass_update_trigger_params(nvl(:new.brkey,
                                                            ...
                                                            'USERBRDG',
                                                            'POSTED_LOAD_C',
                                                            TO_CHAR(:old.POSTED_LOAD_C),
                                                            TO_CHAR(:new.POSTED_LOAD_C),
                                                            1,
                                                            nvl(ls_bridge_id,
                                                                '<MISSING>'),
