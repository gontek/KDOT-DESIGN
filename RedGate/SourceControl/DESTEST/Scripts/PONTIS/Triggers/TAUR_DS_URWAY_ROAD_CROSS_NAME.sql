CREATE OR REPLACE TRIGGER pontis.taur_ds_URWAY_ROAD_CROSS_NAME
   after insert or update of ROAD_CROSS_NAME on pontis.USERRWAY for each row
...
-- Generated 2002-02-26 at 09:26
  ...
  WHEN (nvl( new.ROAD_CROSS_NAME, '<MISSING>' ) <>  nvl( old.ROAD_CROSS_NAME, '<MISSING>' ) ) declare
   lb_result boolean;
...
begin
    ...
    lb_result := ksbms_pontis.f_pass_update_trigger_params(
      ...
      'USERRWAY',
      'ROAD_CROSS_NAME',
      trim(upper(:old.ROAD_CROSS_NAME)),
      trim(upper(:new.ROAD_CROSS_NAME)),
      3,
