CREATE MATERIALIZED VIEW pontis.v_bif_capital_prj_history (brkey,prjctid,actvtyid,actvtydscr,actvtydate,actvtyyear,micro_no,micro_frame,actvdate,bif_micro)
REFRESH COMPLETE START WITH TO_DATE('2016-5-18 10:18:43', 'yyyy-mm-dd hh24:mi:ss') NEXT SYSDATE + (DECODE(TO_CHAR(SYSDATE,'D'),6,3,7,2,1)) 
AS select substr(a.str_name,2,3)||substr(a.str_name,8,3) brkey,
 ...
 from nm3.can_v_str_brid@canp.world a, nm3.can_v_str_actv@canp.world b
    where 1=1
    and b.str_top_id = a.str_id
   and a.str_end_date is not null
   and b.str_end_date is not null
