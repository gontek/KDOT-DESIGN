CREATE OR REPLACE TRIGGER pontis.TAUR_DS_USTR_CRIT_NOTE_DK_2
   after insert or update of CRIT_NOTE_DK_2 on pontis.USERSTRUNIT for each row

-----------------------------------------------------------------------------------------------------------------------------------------
-- Trigger: TAUR_DS_USTR_CRIT_NOTE_DK_2
...
-----------------------------------------------------------------------------------------------------------------------------------------
  ...
  DISABLE WHEN (nvl( new.CRIT_NOTE_DK_2, '<MISSING>' ) <>  nvl( old.CRIT_NOTE_DK_2, '<MISSING>' ) ) declare
   lb_result boolean;
...
begin
 ...
 /*     nvl( :new.brkey, '<MISSING>' ) || ',' || ksbms_pontis.f_strunitlabel_or_strunitkey( null, :new.strunitkey ),
      ...
      'TAUR_DS_USTR_CRIT_NOTE_DK_2');
	 if lb_result
	 then
		  dbms_output.put_line( 'TAUR_DS_USTR_CRIT_NOTE_DK_2 failed' );
	 end if;
end TAUR_DS_USTR_CRIT_NOTE_DK_2;
/