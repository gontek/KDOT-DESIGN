CREATE OR REPLACE TRIGGER pontis.taur_ds_USTR_DECK_THICK
   after insert or update of DECK_THICK on pontis.USERSTRUNIT for each row
...
-- return immediately if not and store nothing in the change log.
 ...
 WHEN (nvl( new.DECK_THICK, -9 ) <>  nvl( old.DECK_THICK, -9 ) ) declare
  lb_result    boolean;
  ls_bridge_id bridge.bridge_id%type;
begin

  -- new code
  IF Ksbms_Util.f_Numbers_Differ(Nvl(:Old.DECK_THICK, -9),
                                 Nvl(:New.DECK_THICK, -9),
