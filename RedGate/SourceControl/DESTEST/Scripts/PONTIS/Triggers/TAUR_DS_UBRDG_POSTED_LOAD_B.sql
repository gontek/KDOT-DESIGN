CREATE OR REPLACE Trigger pontis.TAUR_DS_UBRDG_POSTED_LOAD_B
   after insert or update of POSTED_LOAD_B on pontis.USERBRDG for each row
...
--    2002.10.09  - Deb Kossler, KDOT -- Field changed from text to float to allow for metric conversion.
--                                       Updated trigger by adding to_char for conversion for ds_change_log and changed 'missing's to '-9'
...
when (nvl( new.POSTED_LOAD_B, '-9' ) <>  nvl( old.POSTED_LOAD_B, '-9' ) )
declare
  lb_result    boolean;
...
begin
  ...
  lb_result    := ksbms_pontis.f_pass_update_trigger_params(nvl(:new.brkey,
                                                            ...
                                                            'USERBRDG',
                                                            'POSTED_LOAD_B',
                                                            TO_CHAR(:old.POSTED_LOAD_B),
                                                            TO_CHAR(:new.POSTED_LOAD_B),
                                                            1,
                                                            nvl(ls_bridge_id,
                                                                '<MISSING>'),
