CREATE OR REPLACE TRIGGER pontis.TAUR_DS_URWAY_LN6_VCLR_E
   after insert or update of LN6_VCLR_E on pontis.USERRWAY for each row

-----------------------------------------------------------------------------------------------------------------------------------------
-- Trigger: taur_ds_URWAY_LN6_VCLR_E
...
-- Generated 2006-07-12
 ...
 WHEN (nvl( new.LN6_VCLR_E, -9 ) <>  nvl( old.LN6_VCLR_E, -9 ) ) declare
   lb_result boolean;
   ls_bridge_id bridge.bridge_id%type;
begin
  
  -- new code
  IF Ksbms_Util.f_Numbers_Differ(Nvl(:Old.LN6_VCLR_E, -9),
                                 Nvl(:New.LN6_VCLR_E, -9),
  ...
  -- end change 
    ...
    lb_result := ksbms_pontis.f_pass_update_trigger_params(
      nvl( :new.brkey, '<MISSING>' ) || ',' || ksbms_pontis.f_clr_route( :new.clr_route, :new.on_under )|| ',' ||'E',
      'BRIDGE_ID,CLR_ROUTE,DIRECTION',
      'USERRWAY',
      'LN6_VCLR_E',
      TO_CHAR( :old.LN6_VCLR_E ),
      TO_CHAR( :new.LN6_VCLR_E ),
      6,
      nvl( ls_bridge_id, '<MISSING>' ),
      'taur_ds_URWAY_LN6_VCLR_E');
	 if lb_result
	 then
		  dbms_output.put_line( 'taur_ds_URWAY_LN6_VCLR_E failed' );
	 end if;
end taur_ds_URWAY_LN6_VCLR_E;
/