CREATE OR REPLACE TRIGGER pontis.TAUR_DS_URWAY_LN5_VCLR_N
   after insert or update of LN5_VCLR_N on pontis.USERRWAY for each row

-----------------------------------------------------------------------------------------------------------------------------------------
-- Trigger: taur_ds_URWAY_LN5_VCLR_N
...
-- Generated 2006-07-12
 ...
 WHEN (nvl( new.LN5_VCLR_N, -9 ) <>  nvl( old.LN5_VCLR_N, -9 ) ) declare
  lb_result    boolean;
  ls_bridge_id bridge.bridge_id%type;
begin

  -- new code
  IF Ksbms_Util.f_Numbers_Differ(Nvl(:Old.LN5_VCLR_N, -9),
                                 Nvl(:New.LN5_VCLR_N, -9),
  ...
  lb_result    := ksbms_pontis.f_pass_update_trigger_params(nvl(:new.brkey,
                                                            ...
                                                            'USERRWAY',
                                                            'LN5_VCLR_N',
                                                            TO_CHAR(:old.LN5_VCLR_N),
                                                            TO_CHAR(:new.LN5_VCLR_N),
                                                            6,
                                                            nvl(ls_bridge_id,
                                                                '<MISSING>'),
                                                            'taur_ds_URWAY_LN5_VCLR_N');
  if lb_result then
    dbms_output.put_line('taur_ds_URWAY_LN5_VCLR_N failed');
  end if;
end taur_ds_URWAY_LN5_VCLR_N;
/