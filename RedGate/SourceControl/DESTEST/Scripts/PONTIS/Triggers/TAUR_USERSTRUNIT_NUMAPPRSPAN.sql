CREATE OR REPLACE TRIGGER pontis.TAUR_USERSTRUNIT_NUMAPPRSPAN
...
DISABLE DECLARE V_STRUNITTYPE STRUCTURE_UNIT.STRUNITTYPE%TYPE;
	        V_APPSPANS_NBI BRIDGE.APPSPANS%TYPE;
	        TMP_NUM_SPANS_GRP_1 BRIDGE.APPSPANS%TYPE;
	        TMP_NUM_SPANS_GRP_2 BRIDGE.APPSPANS%TYPE;
	        TMP_NUM_SPANS_GRP_3 BRIDGE.APPSPANS%TYPE;
	        TMP_NUM_SPANS_GRP_4 BRIDGE.APPSPANS%TYPE;
	        TMP_NUM_SPANS_GRP_5 BRIDGE.APPSPANS%TYPE;
	        TMP_NUM_SPANS_GRP_6 BRIDGE.APPSPANS%TYPE;
	        TMP_NUM_SPANS_GRP_7 BRIDGE.APPSPANS%TYPE;
	        TMP_NUM_SPANS_GRP_8 BRIDGE.APPSPANS%TYPE;
	        TMP_NUM_SPANS_GRP_9 BRIDGE.APPSPANS%TYPE;
	        TMP_NUM_SPANS_GRP_10 BRIDGE.APPSPANS%TYPE;
BEGIN

--INITIALIZE to 0 to cover structures without approaches at all.
UPDATE BRIDGE
SET APPSPANS = 0
	WHERE
...
-- Check to See if this any unit other than MAIN
...
V_APPSPANS_NBI := (TMP_NUM_SPANS_GRP_1+TMP_NUM_SPANS_GRP_2+TMP_NUM_SPANS_GRP_3+TMP_NUM_SPANS_GRP_4+TMP_NUM_SPANS_GRP_5+TMP_NUM_SPANS_GRP_6+TMP_NUM_SPANS_GRP_7+TMP_NUM_SPANS_GRP_8+TMP_NUM_SPANS_GRP_9+TMP_NUM_SPANS_GRP_10);


-- Now, Put This Sum of Main Spans into the Pontis BRIDGE Table;
UPDATE bridge
	SET APPSPANS = V_APPSPANS_NBI
	WHERE
	BRIDGE.BRKEY = :OLD.BRKEY;
end if;

END TAUR_USERSTRUNIT_NUMAPPRSPAN;
/