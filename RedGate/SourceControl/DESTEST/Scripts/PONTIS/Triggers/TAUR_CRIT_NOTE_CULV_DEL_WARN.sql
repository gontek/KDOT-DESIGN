...
----------------------------------------------------------------------------------------------------------------------------------------
  ...
  DISABLE WHEN ((nvl( new.crit_note_cul_1, '<MISSING>' ) <>  nvl( old.crit_note_cul_1, '<MISSING>' )) or
...
begin
 ...
 THEN
     ...
     -- send deleted critical notation statement as mail message
       ...
       if ksbms_util.f_email( ls_email_list, 'A '||ls_crit_desc_culv_2|| ' critical notation for Culvert 2 for Bridge '||:old.brkey||' Unit '||:old.strunitkey||' was deleted from the database by user '|| ls_user || ' on ' || to_char(sysdate)||'.  PLEASE NOTE: This crit notation needs to be end-dated in CANSYS.', 'ATTENTION: A Critical Notation was deleted for Bridge '||:old.brkey ) then
            raise lex_email_failed;
       end if;       
