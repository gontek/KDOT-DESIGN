...
--  2015-7-30 deb...no longer need to treat the KTA's special...normal inspection frequency..however
--  still use this trigger to populate updates of brinspfreq_kdot to inspevnt.brinspfreq and inspevnt.elinspfreqb appropriately.
-----------------------------------------------------------------------------------------------------------------------------------------
...
*/
  ...
  WHEN (   (NEW.brinspfreq_kdot <> OLD.brinspfreq_kdot)
...
/*
            -- might be > 99 months, so set to 99 UPDATE 7/30/2015 by dk...this doesn't happen anymore, KTA's are inspected by us normal frequency.
...
--               When changed to  a good months value, sets OSINSPREQ to 'Y' per NBI as well (maybe a  phony change if just changing frequency, but harmless)
         UPDATE inspevnt
            SET brinspfreq = v_insp_freq_months, elinspfreq = v_insp_freq_months
          WHERE inspevnt.brkey = :OLD.brkey
            AND inspevnt.inspkey = :OLD.inspkey;
      ELSE 
      
         UPDATE inspevnt
            SET brinspfreq = NULL, elinspfreq = NULL -- 'blank' per NBI
          WHERE inspevnt.brkey = :OLD.brkey
            AND inspevnt.inspkey = :OLD.inspkey;
