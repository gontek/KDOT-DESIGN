CREATE OR REPLACE Trigger pontis.TAUR_DS_UBRDG_IRLOAD_HL93
    after insert or update of IRLOAD_HL93
    on pontis.USERBRDG for each row

-----------------------------------------------------------------------------------------------------------------------------------------
-- Trigger: TAUR_DS_UBRDG_IRLOAD_HL93
...
when (nvl( new.IRLOAD_HL93, -9 ) <>  nvl( old.IRLOAD_HL93, -9 ) )
declare
  lb_result    boolean;
  ls_bridge_id bridge.bridge_id%type;
begin
  -- new code
  IF Ksbms_Util.f_Numbers_Differ(Nvl(:Old.IRLOAD_HL93, -9),
                                 Nvl(:New.IRLOAD_HL93, -9),
  ...
  lb_result    := ksbms_pontis.f_pass_update_trigger_params(nvl(:new.brkey,
                                                            ...
                                                            'USERBRDG',
                                                            'IRLOAD_HL93',
                                                            TO_CHAR(:old.IRLOAD_HL93),
                                                            TO_CHAR(:new.IRLOAD_HL93),
                                                            1,
                                                            nvl(ls_bridge_id,
                                                                '<MISSING>'),
                                                            'TAUR_DS_UBRDG_IRLOAD_HL93');
  if lb_result then
    dbms_output.put_line('TAUR_DS_UBRDG_IRLOAD_HL93 failed');
  end if;
end TAUR_DS_UBRDG_IRLOAD_HL93;
/