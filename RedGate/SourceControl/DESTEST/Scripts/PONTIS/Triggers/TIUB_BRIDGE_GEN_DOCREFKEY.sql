CREATE OR REPLACE TRIGGER pontis.TIUB_BRIDGE_GEN_DOCREFKEY
  before insert or update on pontis.bridge  
  for each row

DISABLE declare
  /*
  ======================================================================================================  
  TIUB_BRIDGE_GEN_DOCREFKEY - PREUPDATE AND INSERT TRIGGER generates DOCREFKEY 
  for updated or inserted records on BRIDGE
  ======================================================================================================
  -- REVISION HISTORY
  ...
  -- uses a combination of BRKEY + SYS_GUID() to generate DOCREFKEY values
  -- conveniently, this makes the DOCREFKEY searchable with wild cards to match a parent INSPEVNT record
  -- e.g. select from MULTIMEDIA WHERE DOCREFKEY LIKE BRKEY||'%';
  ...
  */
  ...
  ls_dummy                BRIDGE.DOCREFKEY%TYPE;
  ...
  if ls_do_clientside_fixups = 'N' or ls_do_clientside_fixups = 'SERVER' THEN
      ...
      BEGIN
        ...
        */
        ...
        /* Option 2 - Check again and if still null for some reason, take SYS_GUID() random stringset to SYS_GUID() if still null */
          ...
          SELECT :new.BRKEY||SYS_GUID() INTO :new.DOCREFKEY FROM DUAL;
    ...
    EXCEPTION
      ...
      WHEN OTHERS THEN
        ...
        RAISE;
    END UNIQUE_CHECK;
      END IF;  -- DOCREFKEY is BAD, SO FIXED
  ...
  WHEN DOCREFKEY_IS_NULL THEN
    BEGIN
      RAISE_APPLICATION_ERROR(-20203,
                              'Fatal: A new DOCREFKEY value could not be generated - Trigger TIUB_BRIDGE_GEN_DOCREFKEY failed');
    END;
  -- duplicate somehow even though we are calling SYS_GUID()
  WHEN DOCREFKEY_EXISTS_ALREADY THEN
    BEGIN
      RAISE_APPLICATION_ERROR(-20204,
                              'Fatal: The new generated DOCREFKEY value exists in the database already - not unique - Trigger TIUB_BRIDGE_GEN_DOCREFKEY failed');
    END;
  ...
  WHEN MISSING_COPTION THEN
    BEGIN
      RAISE_APPLICATION_ERROR(-20205,
                              'Fatal: The COPTION entry AUTOFIXDOCREFKEY is entirely missing from the database - check COPTIONS table access privileges.  Trigger TIUB_BRIDGE_GEN_DOCREFKEY failed');
    END;
  
  WHEN OTHERS THEN
    BEGIN
      RAISE_APPLICATION_ERROR(-20299,
                              'Fatal: UNKNOWN ERROR condition occurred - Trigger TIUB_BRIDGE_GEN_DOCREFKEY failed  ');
    END;
  


end TIUB_BRIDGE_GEN_DOCREFKEY;
/