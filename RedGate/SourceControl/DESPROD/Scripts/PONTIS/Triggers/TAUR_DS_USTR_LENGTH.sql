CREATE OR REPLACE TRIGGER pontis.taur_ds_USTR_LENGTH
   after insert or update of LENGTH on pontis.USERSTRUNIT for each row
...
-- return immediately if not and store nothing in the change log.
 ...
 WHEN (nvl( new.LENGTH, -9 ) <>  nvl( old.LENGTH, -9 ) ) declare
  lb_result    boolean;
