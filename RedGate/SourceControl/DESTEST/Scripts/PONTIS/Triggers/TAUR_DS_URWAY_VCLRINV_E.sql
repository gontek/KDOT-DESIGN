CREATE OR REPLACE TRIGGER pontis.taur_ds_URWAY_VCLRINV_E
   after insert or update of VCLRINV_E on pontis.USERRWAY for each row
...
-- 2002.07.03 - added direction as a key - same change in VCLRINV_N,S, W and  for other fields that use keymap 5 & 6 in DS_TRANSFER_KEY_MAP
-- requires a new row in DS_TRANSFER_KEY_MAP # 3 for DIRECTION
...
-- Generated 2002-02-26 at 09:26
 ...
 WHEN (nvl( new.VCLRINV_E, -9 ) <>  nvl( old.VCLRINV_E, -9 ) ) declare
  lb_result    boolean;
  ls_bridge_id bridge.bridge_id%type;
begin

  -- new code
  IF Ksbms_Util.f_Numbers_Differ(Nvl(:Old.VCLRINV_E, -9),
                                 Nvl(:New.VCLRINV_E, -9),
