CREATE OR REPLACE TRIGGER pontis.taur_ds_USTR_ABUT_TYPE_NEAR
   after insert or update of ABUT_TYPE_NEAR on pontis.USERSTRUNIT for each row
...
-- Generated 2002-02-26 at 09:26
  ...
  DISABLE WHEN (nvl( new.ABUT_TYPE_NEAR, '<MISSING>' ) <>  nvl( old.ABUT_TYPE_NEAR, '<MISSING>' ) ) declare
   lb_result boolean;
...
/*      nvl( :new.brkey, '<MISSING>' ) || ',' || ksbms_pontis.f_strunitlabel_or_strunitkey( null, :new.strunitkey ),
      'BRIDGE_ID,STRUNITLABEL',*/   nvl( ls_bridge_id, '<MISSING>' ) || ',' || to_char( :new.strunitkey ),
