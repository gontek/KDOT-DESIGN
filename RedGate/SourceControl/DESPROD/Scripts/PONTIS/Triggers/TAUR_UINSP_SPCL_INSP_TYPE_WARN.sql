...
-----------------------------------------------------------------------------------------------------------------------------------------
  ...
  WHEN ( nvl(new.spcl_insp_type, '<MISSING>' ) <>  nvl( old.spcl_insp_type, '<MISSING>' )) 
...
begin
     ...
     ls_user := pontis.f_get_username;
      ...
      -- send warning statement as mail message
       ...
       if ksbms_util.f_email( ls_email_list, 'An "Other Special" inspection type has been added to structure '||:old.brkey||' by user '|| ls_user || ' on ' || to_char(sysdate)||'.  Do not forget to add this info to bridge-level notes.', 'ATTENTION: An "Other Special" inspection type has been added to '||:old.brkey||'!! Add this info to bridge-level notes!') then 
