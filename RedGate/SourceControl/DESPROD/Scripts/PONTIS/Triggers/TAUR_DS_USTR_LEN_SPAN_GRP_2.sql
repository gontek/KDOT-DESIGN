CREATE OR REPLACE TRIGGER pontis.taur_ds_USTR_LEN_SPAN_GRP_2
   after insert or update of LEN_SPAN_GRP_2 on pontis.USERSTRUNIT for each row
...
-- return immediately if not and store nothing in the change log.
 ...
 WHEN (nvl( new.LEN_SPAN_GRP_2, -9 ) <>  nvl( old.LEN_SPAN_GRP_2, -9 ) ) declare
  lb_result    boolean;
  ls_bridge_id bridge.bridge_id%type;
begin

  -- new code
  IF Ksbms_Util.f_Numbers_Differ(Nvl(:Old.LEN_SPAN_GRP_2, -9),
                                 Nvl(:New.LEN_SPAN_GRP_2, -9),
