CREATE OR REPLACE TRIGGER pontis.TAUR_UINSP_NULLVALUE_WARN
...
begin
   ...
   if (:new.oppostcl_kdot is null)

   then

     -- send deleted inspfreq_kdot statement as mail message

       if ksbms_util.f_email( ls_email_list, 'A posting status for '||:old.brkey||' was deleted by user '|| USER || ' on ' || to_char(sysdate)||'.  Is this correct?  Data needs to be verified.', 'ATTENTION: A posting status has been deleted for Bridge '||:old.brkey ) then
