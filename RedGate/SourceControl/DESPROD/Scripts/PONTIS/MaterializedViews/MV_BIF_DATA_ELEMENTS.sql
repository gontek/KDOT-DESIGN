...
REFRESH START WITH TO_DATE('2016-5-19 5:35:30', 'yyyy-mm-dd hh24:mi:ss') NEXT SYSDATE+(DECODE(TO_CHAR(SYSDATE,'D'),6,3,7,2,1)) 
AS SELECT B.BRKEY,
       B.elem_key,
       b.elem_parent_key,
       B.ENVKEY,
       ...
       --    p.factor,
       case
         when B.elem_key in ('520','6000') then
          to_char(round(b.elem_quantity / .3048))
         when b.elem_key in ('510','515') then
       ...
       case
         ...
         when b.elem_key in ('1130') and b.elem_qtystate1 > 0 then
          '1'
         else
       ...
       case
         ...
         when b.elem_key in ('1130') and b.elem_qtystate2 <> 0 then
          '1'
         else
       ...
       case
         ...
         when b.elem_key in ('1130') and b.elem_qtystate3 > 0 then
          '1'
         else
       ...
       case
         ...
         when b.elem_key in ('1130') and b.elem_qtystate4 > 0 then
          '1'
         else
       ...
       P.ENGLISHUNIT,
       '_____' e_line,
       f_get_kdot_element_type(us.brkey,us.strunitkey,b.elem_key) elemtype,
       /*case
