CREATE OR REPLACE TRIGGER pontis.taur_ds_URWAY_HCLRURT_W
   after insert or update of HCLRURT_W on pontis.USERRWAY for each row
...
--  REALLY added directional indicator for use by HIGHWAYS...
...
-- Generated 2002-02-26 at 09:26
 ...
 WHEN (nvl( new.HCLRURT_W, -9 ) <>  nvl( old.HCLRURT_W, -9 ) ) declare
   lb_result boolean;
   ls_bridge_id bridge.bridge_id%type;
begin
   -- new code
  IF Ksbms_Util.f_Numbers_Differ(Nvl(:Old.HCLRURT_W, -9),
                                 Nvl(:New.HCLRURT_W, -9),
  ...
  -- end change  
    ...
    lb_result := ksbms_pontis.f_pass_update_trigger_params(
      nvl( :new.brkey, '<MISSING>' ) || ',' || ksbms_pontis.f_clr_route( :new.clr_route, :new.on_under )|| ',' || 'W',
