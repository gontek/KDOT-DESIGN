CREATE OR REPLACE TRIGGER pontis.taur_ds_INSPEVNT_SUFF_RATE
   after insert or update of SUFF_RATE on pontis.INSPEVNT for each row

-----------------------------------------------------------------------------------------------------------------------------------------
-- Trigger: taur_ds_INSPEVNT_SUFF_RATE
...
-- Generated 2002-02-26 at 09:26
  ...
  DISABLE WHEN (nvl( new.SUFF_RATE, -9 ) <>  nvl( old.SUFF_RATE, -9 ) ) declare
   lb_result boolean;
...
begin
    ...
    lb_result := ksbms_pontis.f_pass_update_trigger_params(
      ...
      'taur_ds_INSPEVNT_SUFF_RATE');
	 if lb_result
	 then
		  dbms_output.put_line( 'taur_ds_INSPEVNT_SUFF_RATE failed' );
	 end if;
end taur_ds_INSPEVNT_SUFF_RATE;
/