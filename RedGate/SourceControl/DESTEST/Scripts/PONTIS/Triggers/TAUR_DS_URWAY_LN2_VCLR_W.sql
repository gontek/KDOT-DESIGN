CREATE OR REPLACE TRIGGER pontis.TAUR_DS_URWAY_LN2_VCLR_W
   after insert or update of LN2_VCLR_W on pontis.USERRWAY for each row

-----------------------------------------------------------------------------------------------------------------------------------------
-- Trigger: taur_ds_URWAY_LN2_VCLR_W
...
-- Generated 2006-07-12
 ...
 WHEN (nvl( new.LN2_VCLR_W, -9 ) <>  nvl( old.LN2_VCLR_W, -9 ) ) declare
  lb_result    boolean;
  ls_bridge_id bridge.bridge_id%type;
begin

  -- new code
  IF Ksbms_Util.f_Numbers_Differ(Nvl(:Old.LN2_VCLR_W, -9),
                                 Nvl(:New.LN2_VCLR_W, -9),
  ...
  lb_result    := ksbms_pontis.f_pass_update_trigger_params(nvl(:new.brkey,
                                                            ...
                                                            'USERRWAY',
                                                            'LN2_VCLR_W',
                                                            TO_CHAR(:old.LN2_VCLR_W),
                                                            TO_CHAR(:new.LN2_VCLR_W),
                                                            6,
                                                            nvl(ls_bridge_id,
                                                                '<MISSING>'),
                                                            'taur_ds_URWAY_LN2_VCLR_W');
  if lb_result then
    dbms_output.put_line('taur_ds_URWAY_LN2_VCLR_W failed');
  end if;
end taur_ds_URWAY_LN2_VCLR_W;
/