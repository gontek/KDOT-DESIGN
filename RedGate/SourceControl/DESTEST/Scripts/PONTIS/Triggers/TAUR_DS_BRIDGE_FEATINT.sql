CREATE OR REPLACE TRIGGER pontis.taur_ds_BRIDGE_FEATINT
   after insert or update of FEATINT on pontis.BRIDGE for each row
...
-- Generated 2002-02-26 at 09:26
 ...
 DISABLE WHEN (nvl( new.FEATINT, '<MISSING>' ) <>  nvl( old.FEATINT, '<MISSING>' ) ) declare
   lb_result boolean;
...
begin
    ...
    lb_result := ksbms_pontis.f_pass_update_trigger_params(
      ...
      'BRIDGE',
      'FEATINT',
      TRIM(UPPER(:old.FEATINT)),
      trim(UPPER(:new.FEATINT)),
      1,
