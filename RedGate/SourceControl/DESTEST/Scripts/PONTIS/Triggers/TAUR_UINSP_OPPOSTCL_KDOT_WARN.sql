...
begin
     ...
     ls_email_list := nvl( ksbms_util.f_get_coption_value('EMAIL_OPPOSTCL_WARN_LIST'),'deb@ksdot.org');
 

 if ( :new.oppostcl_kdot = '1') and
   user not in ('KSBMS_ROBOT') and
   ...
   then
     ...
     -- send warning statement as mail message
       ...
       if ksbms_util.f_email( ls_email_list, 'Unexpected change made to Open_Posted_Closed for brkey '||:old.brkey||' by user '|| USER || ' on ' || to_char(sysdate)||'.  If OPENING do not forget additional fields to fix in Cansys, BOB!!', 'ATTENTION: An unexpected change to Open/Posted/Closed Status in Pontis has occurred!!!!  See Bob about bridge '||:old.brkey ) then 
