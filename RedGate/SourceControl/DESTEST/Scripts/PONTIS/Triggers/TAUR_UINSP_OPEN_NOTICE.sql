...
-----------------------------------------------------------------------------------------------------------------------------------------
  ...
  DISABLE WHEN ( nvl(new.oppostcl_kdot, '<MISSING>' ) <>  nvl( old.oppostcl_kdot, '<MISSING>' )) declare
...
begin
     ...
     ls_email_list := nvl( ksbms_util.f_get_coption_value('EMAIL_BRIDGE_OPEN_LIST'),'deb@ksdot.org');
     v_descrip_old := f_get_paramtrs_equiv_long( 'USERINSP', 'OPPOSTCL_KDOT',:old.OPPOSTCL_KDOT );
     v_descrip_new := f_get_paramtrs_equiv_long( 'USERINSP', 'oppostcl_kdot',:new.oppostcl_kdot);

 if ( :new.oppostcl_kdot = '1' ) and
   user = 'KSBMS_ROBOT'
   ...
   then
     ...
     -- send warning statement as mail message
       ...
       if ksbms_util.f_email( ls_email_list, 'Bridge status changed from '||'"'||v_descrip_old||'"'||' to '||'"'||v_descrip_new||'"'||' for brkey '||:old.brkey||' by user '|| USER || ' on ' || to_char(sysdate)||'. ', 'ATTENTION: Bridge '||:old.brkey||' has been updated to '||'"'||v_descrip_new||'"'||' status!!' ) then 
