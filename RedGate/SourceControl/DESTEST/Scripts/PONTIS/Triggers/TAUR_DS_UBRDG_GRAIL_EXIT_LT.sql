CREATE OR REPLACE TRIGGER pontis.taur_ds_UBRDG_GRAIL_EXIT_LT
   after insert or update of GRAIL_EXIT_LT on pontis.USERBRDG for each row

-----------------------------------------------------------------------------------------------------------------------------------------
-- Trigger: taur_ds_UBRDG_GRAIL_EXIT_LT
...
-- Generated 2002-02-26 at 09:26
 ...
 WHEN (nvl( new.GRAIL_EXIT_LT, '<MISSING>' ) <>  nvl( old.GRAIL_EXIT_LT, '<MISSING>' ) ) declare
   lb_result boolean;
...
begin
    ...
    lb_result := ksbms_pontis.f_pass_update_trigger_params(
      ...
      'taur_ds_UBRDG_GRAIL_EXIT_LT');
	 if lb_result
	 then
		  dbms_output.put_line( 'taur_ds_UBRDG_GRAIL_EXIT_LT failed' );
	 end if;
end taur_ds_UBRDG_GRAIL_EXIT_LT;
/