CREATE OR REPLACE TRIGGER pontis.TAUR_DS_UINSP_CULV_COMP_HI
  after insert or update on pontis.userinsp  
  for each row
-----------------------------------------------------------------------------------------------------------------------------------------
-- Trigger: taur_ds_UINSP_CULV_COMP_HI
...
----------------------------------------------------------------------------------------------------------------------------------------
  ...
  DISABLE WHEN (nvl( new.CULV_COMP_HI, -9 ) <>  nvl( old.CULV_COMP_HI, -9 ) ) declare
   lb_result boolean;
...
if (substr(ls_bridge_id,6,1)='B') then --only bridges not 500 culverts at this time dk
    lb_result := ksbms_pontis.f_pass_update_trigger_params(
      ...
      'USERINSP',
      'CULV_COMP_HI',
      TO_CHAR( :old.CULV_COMP_HI ),
      TO_CHAR( :new.CULV_COMP_HI ),
      1,
      nvl( ls_bridge_id, '<MISSING>' ),
      'taur_ds_UINSP_CULV_COMP_HI');
end if;
	 if lb_result
	 then
		  dbms_output.put_line( 'taur_ds_UINSP_CULV_COMP_HI failed' );
	 end if;

end TAUR_DS_UINSP_CULV_COMP_HI;
/