...
-----------------------------------------------------------------------------------------------------------------------------------------
  ...
  WHEN (nvl (new.uwater_insp_typ, '<MISSING>') <> nvl
  (old.uwater_insp_typ, '<MISSING>'))
declare
...
begin
  ...
  if (:old.uwater_insp_typ in ('3', '4') and
     (:new.uwater_insp_typ not in ('3', '4')) or
     (:old.uwater_insp_typ not in ('3', '4') and
     (:new.uwater_insp_typ in ('3', '4'))))
   ...
   then
    ...
    if ksbms_util.f_email(ls_email_list,
                          'The underwater inspection type has been changed from ' ||
                          f_get_paramtrs_equiv('userinsp',
                                               'uwater_insp_typ',
                                               :old.uwater_insp_typ) ||
                          ' to ' ||
                          f_get_paramtrs_equiv('userinsp',
                                               'uwater_insp_typ',
                                               :new.uwater_insp_typ) ||
                          ' for structure ' || :old.brkey || ' by user ' || ls_user ||
                          ' on ' || to_char(sysdate) ||
                          '.  Make sure this is correct for this structure. PLEASE NOTE...IF THE TYPE HAS CHANGED TO "9" then DELETE INTERVAL FROM CANSYS...!!',
                          'ATTENTION: An underwater inspection type has been changed on ' ||
                          :old.brkey || '!! Make sure this is correct.') then
