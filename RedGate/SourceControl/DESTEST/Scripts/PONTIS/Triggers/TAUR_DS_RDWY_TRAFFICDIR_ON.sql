CREATE OR REPLACE TRIGGER pontis.TAUR_DS_RDWY_TRAFFICDIR_ON
   after insert or update of TRAFFICDIR on pontis.ROADWAY for each row

-----------------------------------------------------------------------------------------------------------------------------------------
-- Trigger: TAUR_DS_RDWY_TRAFFICDIR_ON
...
--               these updates go the Cansys FUNC - on-records only----------------------------------------------------------------------
  ...
  DISABLE WHEN (nvl( new.trafficdir, '<MISSING>' ) <>  nvl( old.trafficdir, '<MISSING>' )and NEW.ON_UNDER = '1' ) declare
   lb_result boolean;
...
begin
    ...
    lb_result := ksbms_pontis.f_pass_update_trigger_params(
      ...
      'TAUR_DS_RDWY_TRAFFICDIR_ON');
   if lb_result
   then
      dbms_output.put_line( 'TAUR_DS_RDWY_TRAFFICDIR_ON failed' );
   end if;
end TAUR_DS_RDWY_TRAFFICDIR_ON;
/