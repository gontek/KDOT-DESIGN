CREATE OR REPLACE Trigger pontis.taur_ds_UBRDG_ATTACH_DESC_2
   after insert or update of ATTACH_DESC_2 on pontis.USERBRDG for each row
...
nvl( new.ATTACH_DESC_2, '<MISSING>' ) <>  nvl( old.ATTACH_DESC_2, '<MISSING>' ) )
declare
   lb_result boolean;
...
begin
    ...
    lb_result := ksbms_pontis.f_pass_update_trigger_params(
      ...
      'USERBRDG',
      'ATTACH_DESC_2',
      trim(upper(:old.ATTACH_DESC_2)),
      trim(upper(:new.ATTACH_DESC_2)), -- get rid of extra spaces
      11,-- Allen Marshall, CS - 2002.07.08 - key map number 111,
