...
(
 SELECT   bridge.brkey,
         ...
         nvl(userbrdg.kta_id,'_') kta_id,
         inspevnt.inspdate as R_Date,
         extract(year from nextinsp) as R_YEARDUE,
         decode(userinsp.brinspfreq_kdot,99,12,userinsp.brinspfreq_kdot) as R_Frq,
         inspevnt.nextinsp,
         ...
         case when trunc(sysdate-nextinsp) > 30 and trunc(sysdate-nextinsp) <= 90
          ...
          case when extract(year from nextinsp) = extract(year from sysdate) and
           extract(month from nextinsp)-1 = extract(month from sysdate) then
           'Y'
           else 'N'
         ...
         case when trunc(sysdate-fcnextdate) > 30 and trunc(sysdate-fcnextdate) <= 90
          ...
          case when extract(year from fcnextdate) = extract(year from sysdate) and
           extract(month from fcnextdate)-1 = extract(month from sysdate) then
           'Y'
           else 'N'
         ...
         case when trunc(sysdate-uwnextdate) > 30 and trunc(sysdate-uwnextdate) <= 90
          ...
          case when extract(year from uwnextdate) = extract(year from sysdate) and
           extract(month from uwnextdate)-1 = extract(month from sysdate) then
           'Y'
           else 'N'
   ...
   --      or( inspevnt.inspdate > '01/jan/1901'))
         -- and extract(year from inspdate )+ decode(brinspfreq_kdot,99,1,brinspfreq_kdot) <= extract(year from sysdate)))
     )
         union all
         SELECT   bridge.brkey,
         'F' as inspectype,
   ...
   WHERE userinsp.brkey = bridge.brkey and
     ...
     where u.brkey = bridge.brkey and
           ((u.crit_note_sup_1 = '9') or
            ...
            (u.crit_note_sup_5 = '9'))
      -- and ( inspevnt.oslastinsp > '01/jan/1901' and extract(year from oslastinsp )+ osinspfreq_kdot <= extract(year from sysdate)))
            )
            union all
       SELECT   bridge.brkey,
         'U' as inspectype,
   ...
   WHERE userinsp.brkey = bridge.brkey and
      ...
      inspevnt.inspkey = mv_latest_inspection.inspkey and
      kta_insp in ('0','1') and
      userinsp.uwater_insp_typ in ('2','3','4') and
     (       ( inspevnt.uwlastinsp > '01/jan/1901' and inspevnt.notes is null)
         or( inspevnt.uwlastinsp > '01/jan/1901' and uwnextdate > '01/jan/1901' ))
         --and extract(year from uwlastinsp )+ uwinspfreq_kdot <= extract(year from sysdate)))
         UNION ALL
         SELECT   bridge.brkey,
         'S' as inspectype,
         bridge.district,
   ...
   WHERE userinsp.brkey = bridge.brkey and
     ...
     userinsp.snoop_insp_req = 'Y' and
       nvl(snoop_insp_freq,0) > 0 and nvl(snoop_insp_freq,0) < 99
  --   and    extract(year from snoop_next_insp) <= extract(year from sysdate)
