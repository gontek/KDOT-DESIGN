CREATE OR REPLACE function pontis.f_get_bif_eleminsp_a(v_brkey       eleminsp.brkey%type,
...
begin
  ...
  */
  ...
  sqlString := 'select ltrim(substr(replace(sys_connect_by_path(' ||
              ...
              --into retval
               ...
               ' ,
                            ...
                            row_number() over(partition by brkey,strunitkey order by strunitkey,to_number(' ||
               columnname_in ||
               ' )) seq ' ||
               ' from mv_bif_data_deck_elements) ' ||
               '   where seq = cnt  ' || '   and brkey = :br ' ||
               '   and strunitkey = :su ' || '   start with seq = 1 ' ||
  ...
  execute immediate sqlString
    into retval
    using v_brkey, v_strunitkey; -- these are determined by position in the string (see :br and :su above)
