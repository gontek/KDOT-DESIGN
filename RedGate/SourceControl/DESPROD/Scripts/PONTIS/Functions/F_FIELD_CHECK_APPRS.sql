CREATE OR REPLACE function pontis.f_field_check_apprs(v_brkey bridge.brkey%type)
 -- Summary: Function to gather approach data used for field apprpvmt in view v_field_check_form 
  -- True or False values if an approach exists based on element data<BR>
  -- <BR>
  -- %revision-history
  -- created: 2015-11-13<BR>
  -- revised: 2015-11-13 added documentation<BR>
  -- <p id="doc_save_date" style="margin: 0;">documentation revised: 2015-07-10</p><p id="doc_mod_date" style="margin: 0;"></p>
  -- %copyright-notice Kansas Department of Transportation, 2015 - all rights reserved
  -- %kdot-contact Ms. Deb Kossler, Bureau of Design<BR>
  -- <a href="mailto:deb@ksdot.org?Subject="Documentation%20Question%20re:%f_field_check_apprs">Email questions</a><BR>
  -- %developer-info Deb Kossler, KDOT
    -- %development-environment
  -- Oracle Database 11g Release 11.2.0.4.0<BR>
  -- OCI: version 11.1
  -- %param v_brkey IN VARCHAR2 

  -- %usage SELECT f_field_check_apprs(b.brkey) from bridge b will return a "True" if the element for approach slab exists
  --  or False if it not <br>
  -- ...<br>
  -- ...<br>
  -- result = True or False
  -- ...<br>
  -- ...<br>
  -- used in Calvin's Field Check Form to give designers initial bridge information about a bridge they're going to replace
  -- %raises NO_DATA_FOUND if a match cannot be determined, and returns a generic UNDERSCORE '_' result
  -- or raises a generic exception on OTHER<BR>
 
 return varchar is
  retval varchar2(5);
  v_count number;
  
begin
 select count(elem_key) into
 v_count 
 from mv_bif_data_elements e
 where brkey = v_brkey and
       elem_key in ('320','321');
       
if v_count <=  0 then
  retval := 'False';
  
else 
  retval := 'True';
end if;

   
  return retval;
  
  EXCEPTION
  WHEN No_Data_Found THEN
    RETURN NULL; 
  WHEN OTHERS THEN
    RAISE;
end f_field_check_apprs;
/