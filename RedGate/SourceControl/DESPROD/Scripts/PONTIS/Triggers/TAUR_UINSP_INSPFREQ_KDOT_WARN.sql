...
begin
   ...
   then
     ...
     -- send deleted inspfreq_kdot statement as mail message
       ...
       if ksbms_util.f_email( ls_email_list, 'A routine inspection frequency for '||:old.brkey||' was deleted by user '|| ls_user  || ' on ' || to_char(sysdate)||'.  Is this correct?  Data needs to be verified.', 'ATTENTION: A routine inspection frequency has been deleted for Bridge '||:old.brkey ) then
