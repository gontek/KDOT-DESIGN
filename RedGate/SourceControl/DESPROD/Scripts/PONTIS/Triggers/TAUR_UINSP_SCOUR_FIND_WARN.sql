...
-----------------------------------------------------------------------------------------------------------------------------------------
  ...
  -----------------------------------------------------------------------------------------------------------------------------------------
  WHEN (nvl
  (new.scour_find_date, TO_DATE('1901-01-01', 'YYYY-MM-DD')) <> nvl
  (old.scour_find_date, TO_DATE('1901-01-01', 'YYYY-MM-DD')))
declare
...
begin
  ...
  ls_email_list := nvl(ksbms_util.f_get_coption_value('EMAIL_SCOUR_FIND_WARN_LIST'),
                       'deb@ksdot.org');
  ls_user := pontis.f_get_username;
  
  if ksbms_util.f_email(ls_email_list,
                        'A critical scour finding was noted on ' ||
                        :new.scour_find_date || ' by user ' || ls_user ||
